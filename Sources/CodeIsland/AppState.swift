...
final class AppState {
    ...
    func handleEvent(_ event: HookEvent) {
        ...
        if sessions[sessionId]?.model == nil
            ...
            let model = Self.readModelFromTranscript(sessionId: event.sessionId ?? sessionId, cwd: cwd)
        ...
        for info in discovered {
            ...
            if sessions[sessionKey] != nil {
                if processMonitors[sessionKey] == nil, let pid = info.pid {
                    monitorProcess(sessionId: sessionKey, pid: pid)
                ...
                refreshProviderTitle(for: sessionKey, providerSessionId: info.sessionId)
                continue
            }
